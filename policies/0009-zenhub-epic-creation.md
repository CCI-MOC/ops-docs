...
An Epic is a large body of work or a major feature that's too big to be completed in a single sprint, serving as a high-level container for related, smaller work items called User Stories or Issues. Think of it as a strategic goal or an overarching objective (like "Build a Mobile App") that gets broken down into manageable User Stories (like "User Login," "Product Browsing," "Checkout") that teams can tackle in shorter development cycles (sprints). Epics provide focus, organization, and a clear path for delivering significant value over time.  
...
In the past ZenHub managed their epics as seporate entities and those can still be found if you have the old links. These are no longer used and therfore we will not discuss them further here.

## Policy

ZenHub is where we currently manage our epics and therefore we will be using ZenHub to create our Epics. The way that ZenHub manages Epics is by utilizing a similar process as GitHub. Epics are Issues of Issue Type Epic. This means that you can convert issues to Epics by changing their Issue Type. This can be confusing becuase there are ZenHub Issues and Git Hub Issues and they both create slightly different Epics.
...
2. Create an Epic Template
   ...
   2. Duplicate it
      1. Click the the ellipsis int he upper right ![ellipsis](/policies/assets/ZHellipsis.jpg)
...
This policy is already in affect.
